...
app.get("/", function(req,res){
  // res.send("<h1>Home</h1>");
  res.render("home",{homefirstcontent:homeStartingContent,addedcompose:composeItems})
...
app.get("/post/:postType",function(req,res){
  ...
  let afterPostHeaading = _.lowerCase(req.params.postType)



  composeItems.forEach(function(element){
    let postHeading=_.lowerCase(element.titleContent)
    if(postHeading===afterPostHeaading){

      res.render("post",{singlePostHeading:element.titleContent,singlePostContent:element.postContent})
    }
    else {
      console.log("Not Matching");
    }
